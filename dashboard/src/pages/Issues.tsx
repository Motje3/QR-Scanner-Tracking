...
const Issues = () => {
  ...
  const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || "http://localhost:5070"; // Adjust if different
    ...
    <div className="p-6 space-y-6 bg-gradient-to-br from-indigo-950 via-slate-900 to-purple-950 min-h-[92vh] text-white">
      ...
      {!loading && !error && (
        <div className="space-y-4">
          {/* Table Header */}
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 bg-indigo-900/80 backdrop-blur-sm rounded-lg p-4 text-left text-xs uppercase tracking-wider font-semibold text-indigo-300">
            <div>ID</div>
          ...
          <div className="space-y-3">
            {issues.length === 0 ? (
              ...
              issues.map((issue) => (
                <div
                  key={issue.id}
                  className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4 bg-indigo-800/70 backdrop-blur-sm rounded-lg shadow-md hover:bg-indigo-700/90 hover:shadow-xl transition-all duration-200 ease-in-out"
                ...
                >
                  ...
                  <div className="text-sm text-gray-300 self-center break-words overflow-hidden max-h-20"> {/* Limit description height */}
                    {issue.description || "-"}
                  </div>
