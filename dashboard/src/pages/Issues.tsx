...
import { AlertTriangle, CheckCircle, Search, Star, Lightbulb } from 'lucide-react'; // Icons for cards and buttons
...
const Issues = () => {
  ...
  useEffect(() => {
    const fetchIssueReports = async () => {
      try {
        const response = await fetch(`http://172.20.10.2:5070/api/IssueReport`); // Adjust URL if needed
        if (!response.ok) {
  ...
  const filteredAndSortedIssues = useMemo(() => {
    let filtered = issueReports.filter(issue =>
  ...
  return (
    <div className="space-y-6 p-6">
        ...
        <div className="bg-indigo-900 rounded-lg p-6">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h2 className="text-3xl font-bold text-white">{totalUnsolvedIssues}</h2>
            </div>
            <div className="p-2 bg-indigo-800 rounded-md">
              <AlertTriangle className="text-red-400" size={24} /> {/* Icon for unsolved */}
            </div>
          </div>
        </div>
        ...
        <div className="bg-indigo-900 rounded-lg p-6">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h2 className="text-3xl font-bold text-white">{fixedIssuesToday}</h2>
            </div>
            <div className="p-2 bg-indigo-800 rounded-md">
              <CheckCircle className="text-green-400" size={24} /> {/* Icon for fixed today */}
            </div>
          </div>
        </div>
        ...
        <div className="bg-indigo-900 rounded-lg p-6">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h2 className="text-3xl font-bold text-white">{fixedIssuesTotal}</h2>
            </div>
            <div className="p-2 bg-indigo-800 rounded-md">
              <Lightbulb className="text-yellow-200" size={24} /> {/* Icon for total fixed */}
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Issue List */}
      <div className="bg-indigo-900 rounded-lg p-6">
        <h3 className="text-white font-semibold mb-4">Problemen die nog moeten worden opgelost</h3>
        {filteredAndSortedIssues.length === 0 ? (
          <p className="text-gray-400">Geen problemen gevonden.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredAndSortedIssues.map((issue) => (
              <div key={issue.id} className="bg-[#1E1B33] rounded-lg p-4 flex flex-col justify-between shadow-lg">
                <div>
                  ...
                  {issue.imageUrl && (
                    <img src={issue.imageUrl} alt="Issue" className="w-full h-32 object-cover rounded-md mb-2" />
                  )}
                  {issue.shipmentId && (
                ...
                <div className="flex justify-end gap-2 mt-4">
                  ...
                  >
                    {issue.isImportant ? (
                      <span className="flex items-center"><Star className="mr-1" size={16} /> Belangrijk</span>
                    ) : (
                      <span className="flex items-center"><Star className="mr-1" size={16} /> Markeer als belangrijk</span>
                    )}
                  </button>
                  <button
                  ...
                  >
                    {issue.isFixed ? (
                      <span className="flex items-center"><CheckCircle className="mr-1" size={16} /> Opgelost</span>
                    ) : (
                      <span className="flex items-center"><CheckCircle className="mr-1" size={16} /> Markeer als opgelost</span>
                    )}
                  </button>
                </div>
              </div>
...
const Input = ({ placeholder, value, onChange, className, startContent }: any) => {
    return (
