...
const scrollbarStyles = `
  ...
  .scrollbar-custom::-webkit-scrollbar-thumb:active {
    background: linear-gradient(180deg, #C4B5FD 0%, #A5B4FC 50%, #93C5FD 100%);
  }
  
  /* Firefox */
  .scrollbar-custom {
    scrollbar-width: thin;
    scrollbar-color: #8B5CF6 rgba(79, 70, 229, 0.1);
  }
`;
              ...
              {recentIssues.map((issue) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    {issue.description && (
                      <p
                        className="text-gray-300 text-xs mb-1 line-clamp-1"
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="text-gray-300">
                    {recentShipments.map((shipment) => (
                      <tr
                        key={shipment.id}
                        className="border-b border-indigo-800 hover:bg-indigo-800/30 transition-colors"
                      >
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getShipmentStatusColor(shipment.status)}`}
                          >
                            {shipment.status}
                          </span>
                        </td>
