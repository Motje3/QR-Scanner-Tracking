  ...
  const validateField = (field: string, value: string): string | undefined => {
    switch (field) {
      case 'username':
        ...
        if (value.length < 3) return 'Gebruikersnaam moet minimaal 3 karakters bevatten.';
        if (value.length > 50) return 'Gebruikersnaam mag maximaal 50 karakters bevatten.';
        break;
      case 'password':
        if (!value.trim()) return 'Wachtwoord is verplicht.';
        if (value.length < 6) return 'Wachtwoord moet minimaal 6 karakters bevatten.';
        if (value.length > 100) return 'Wachtwoord mag maximaal 100 karakters bevatten.';
        break;
      case 'fullName':
        if (!value.trim()) return 'Volledige naam is verplicht.';
        if (value.length > 100) return 'Volledige naam mag maximaal 100 karakters bevatten.';
        break;
      case 'email':
        if (value.trim()) {
          const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
          if (!emailRegex.test(value)) return 'Ongeldig emailadres formaat.';
          if (value.length > 100) return 'Email mag maximaal 100 karakters bevatten.';
        }
        break;
      case 'role':
    ...
    try {
      ...
      if (err.response?.data?.errors) {
        ...
        Object.keys(errorData).forEach(key => {
          ...
          if (lowerKey.includes('username')) serverErrors.username = errorData[key][0];
          if (lowerKey.includes('password')) serverErrors.password = errorData[key][0];
          if (lowerKey.includes('fullname')) serverErrors.fullName = errorData[key][0];
          if (lowerKey.includes('email')) serverErrors.email = errorData[key][0];
      ...
      <div className="bg-indigo-900/70 backdrop-blur-md rounded-xl shadow-lg p-4 flex flex-col md:flex-row items-center gap-4">
        <div className="relative flex-grow w-full md:w-auto">
           <Input
            ...
            classNames={{
                inputWrapper: "bg-indigo-800/80 border-indigo-700/50 rounded-lg shadow-sm h-11 group-data-[focus=true]:border-purple-500 group-data-[focus=true]:ring-2 group-data-[focus=true]:ring-purple-500/20 transition-all duration-200 ease-in-out",
                input: "text-white placeholder:text-gray-500 text-sm focus:outline-none",
        ...
        <select className="w-full md:w-auto bg-indigo-800/80 border border-indigo-700/50 text-white px-4 py-2.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/50 text-sm h-11 transition-all duration-200 ease-in-out hover:bg-indigo-700/90 hover:border-indigo-600/60 cursor-pointer appearance-none bg-[url('data:image/svg+xml;base64,')] bg-no-repeat bg-right-3 bg-center pr-10">
          <option value="all" className="bg-indigo-800 text-white hover:bg-indigo-700">Alle Rollen</option>
          <option value="Admin" className="bg-indigo-800 text-white hover:bg-indigo-700">Admin</option>
          <option value="Manager" className="bg-indigo-800 text-white hover:bg-indigo-700">Manager</option>
        </select>
        <select className="w-full md:w-auto bg-indigo-800/80 border border-indigo-700/50 text-white px-4 py-2.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/50 text-sm h-11 transition-all duration-200 ease-in-out hover:bg-indigo-700/90 hover:border-indigo-600/60 cursor-pointer appearance-none bg-[url('data:image/svg+xml;base64,')] bg-no-repeat bg-right-3 bg-center pr-10">
          <option value="all" className="bg-indigo-800 text-white hover:bg-indigo-700">Alle Statussen</option>
          <option value="active" className="bg-indigo-800 text-white hover:bg-indigo-700">Actief</option>
          <option value="inactive" className="bg-indigo-800 text-white hover:bg-indigo-700">Inactief</option>
        </select>
      </div>
          ...
          <div className="bg-indigo-900 rounded-xl shadow-2xl p-6 w-full max-w-lg">
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <input type="text" className="w-full bg-indigo-800 border border-indigo-700 rounded-md p-2.5 text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all duration-200 ease-in-out" defaultValue={selectedAccount.fullName} />
              </div>
              <div>
                <label className="block text-sm text-gray-300 mb-1">Email</label>
                <input type="email" className="w-full bg-indigo-800 border border-indigo-700 rounded-md p-2.5 text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all duration-200 ease-in-out" defaultValue={selectedAccount.email} />
              </div>
              <div>
                <label className="block text-sm text-gray-300 mb-1">Rol</label>
                <select className="w-full bg-indigo-800 border border-indigo-700 rounded-md p-2.5 text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all duration-200 ease-in-out cursor-pointer appearance-none bg-[url('data:image/svg+xml;base64,')] bg-no-repeat bg-right-3 bg-center pr-10" defaultValue={selectedAccount.role}>
                  <option value="Admin" className="bg-indigo-800 text-white">Admin</option> 
                  <option value="Manager" className="bg-indigo-800 text-white">Manager</option> 
                  <option value="User" className="bg-indigo-800 text-white">User</option>
                </select>
              </div>
            </div>
            <div className="flex justify-end space-x-3">
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleCreateAccount(); }} className="space-y-4 mb-6">
              <div>
                ...
                <input 
                  ...
                  className={`w-full bg-indigo-800 border rounded-md p-2.5 text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all duration-200 ease-in-out ${
                    validationErrors.username ? 'border-red-500 focus:border-red-400 focus:ring-red-400/50' : 'border-indigo-700 focus:border-purple-500'
                  }`}
              ...
              <div>
                ...
                <input 
                  ...
                  className={`w-full bg-indigo-800 border rounded-md p-2.5 text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all duration-200 ease-in-out ${
                    validationErrors.fullName ? 'border-red-500 focus:border-red-400 focus:ring-red-400/50' : 'border-indigo-700 focus:border-purple-500'
                  }`}
              ...
              <div>
                ...
                <input 
                  ...
                  className={`w-full bg-indigo-800 border rounded-md p-2.5 text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all duration-200 ease-in-out ${
                    validationErrors.email ? 'border-red-500 focus:border-red-400 focus:ring-red-400/50' : 'border-indigo-700 focus:border-purple-500'
                  }`}
              ...
              <div>
                ...
                <select 
                  ...
                  className={`w-full bg-indigo-800 border rounded-md p-2.5 text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all duration-200 ease-in-out cursor-pointer appearance-none bg-[url('data:image/svg+xml;base64,')] bg-no-repeat bg-right-3 bg-center pr-10 ${
                    validationErrors.role ? 'border-red-500 focus:border-red-400 focus:ring-red-400/50' : 'border-indigo-700 focus:border-purple-500'
                  }`}
                >
                  <option value="" className="bg-indigo-800 text-white">Selecteer een rol</option>
                  <option value="User" className="bg-indigo-800 text-white">User</option>
                  <option value="Admin" className="bg-indigo-800 text-white">Admin</option>
                  <option value="Manager" className="bg-indigo-800 text-white">Manager</option>
                </select>
              ...
              <div>
                ...
                <input 
                  ...
                  className={`w-full bg-indigo-800 border rounded-md p-2.5 text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all duration-200 ease-in-out ${
                    validationErrors.password ? 'border-red-500 focus:border-red-400 focus:ring-red-400/50' : 'border-indigo-700 focus:border-purple-500'
                  }`}
